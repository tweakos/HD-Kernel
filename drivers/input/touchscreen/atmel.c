...
int s2w_switch = 1;
int s2w_temp = 1;
#elif defined(CONFIG_TOUCHSCREEN_ATMEL_TAPTAP2WAKE_ENABLED)
int s2w_switch = 2;
int s2w_temp = 2;
#elif defined(CONFIG_TOUCHSCREEN_ATMEL_2F2WAKE_ENABLED)
int s2w_switch = 3;
int s2w_temp = 3;
#endif
		...
		struct device_attribute *attr, const char *buf, size_t count)
{
	...
	if (s2w_temp == 0) 
		...
		printk(KERN_INFO "[sweep2wake]: TT2WAKE enabled.\n");
        else if (s2w_temp == 3)
	...
	for (loop_i = 0; loop_i < ts->finger_support; loop_i++) {
		if (ts->finger_pressed & BIT(loop_i)) {
			if (ts->id->version >= 0x15) {
...
#ifdef CONFIG_TOUCHSCREEN_ATMEL_SWEEP2WAKE
			    if((ts->finger_count == 2) && (scr_suspended == true) && (s2w_switch == 3)) {							
                if (ts->finger_data[loop_i].y > 980) {
									barrier[0] = true;
									barrier[1] = true;										
											if (exec_count) {
				...
				} else if ((s2w_switch == 1) && (scr_suspended == true) && (ts->finger_count == 1)) {
				prevx = 0;
				nextx = 333;
				if ((barrier[0] == true) ||
			...
			} else if ((ts->finger_count == 1) && (scr_suspended == true) && (s2w_switch == 2)) {
			if (ts->finger_data[loop_i].y > 980) {
								f_pressed = true;								
								//printk(KERN_INFO "finger pressed,ctime:%d",c_time); 
								if (c_time>0) {                              	
								n_time = jiffies/10;
								//printk(KERN_INFO "finger pressed,ntime:%d",n_time);
								if (n_time<=(c_time+TT2Wake)){
	...
	if (ts->id->version >= 0x15) {
		...
		if (!ts->finger_count || ts->face_suppression) {
			...
			if ((((ts->finger_count > 0)?1:0) == 0) && (s2w_switch > 0)) {
				if ((scr_suspended == true) && (f_pressed == true) && (s2w_switch == 2)) {
					c_time=jiffies/10;					
					//printk(KERN_INFO "finger released,time:%d",c_time);
				}
				exec_count = true;
				barrier[0] = false;
				barrier[1] = false;
