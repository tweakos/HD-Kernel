...
bool f_pressed2 = false;
uint c_time2,n_time2;
int TT2Wake2=1;
...
int s2w_switch2 = 1;
int s2w_temp2 = 1;
#elif defined(CONFIG_TOUCHSCREEN_SYNAPTICS_TT2WAKE_ENABLED)
int s2w_switch2 = 2;
int s2w_temp2 = 2;
		...
		if (finger_pressed == 0) {
			...
			input_report_abs(ts->input_dev, ABS_MT_POSITION, 1 << 31);
				...
				if ((((finger_count > 0)?1:0) == 0) && (s2w_switch2 > 0)) {
					...
					if (s2w_switch2 == 2) {
					if ((scr_suspended2 == true)&&
                         (f_pressed2 == true)){					
					c_time2=jiffies/10;					
					//printk(KERN_INFO "finger released,time:%d",c_time2); 
					}
					}
				}
#endif	
		...
		for (loop_i = 0; loop_i < ts->finger_support; loop_i++) {
			...
			if ((((finger_pressed | finger_release_changed) >> loop_i) & 1) == 1) {
                ...
                if ((!wake_lock_active(&sweep2wake_wake_lock)) && 
                    (scr_suspended2==true) && 
                    (finger_count > 0) && 
                    (finger_data[loop_i][0] > 30) &&
					(finger_data[loop_i][1] > 1900)) {
					wake_lock(&sweep2wake_wake_lock);
					printk(KERN_INFO "[sweep2wake]: detected irq, wakelock\n");
				}
							if ((finger_count == 1) && (scr_suspended2 == true) && (s2w_switch2 == 2)) {			
								f_pressed2 = true;								
								//printk(KERN_INFO "finger pressed,ctime:%d",c_time2); 
								if (c_time2>0) {                              	
								n_time2 = jiffies/10;
								//printk(KERN_INFO "finger pressed,ntime:%d",n_time2);
								if (n_time2<=(c_time2+TT2Wake2)){
								//printk(KERN_INFO "tt2w on");
								barrier2[1] = true;
								if (exec_count2) {
							...
							} else if((finger_count == 2) && (scr_suspended2 == true) && (s2w_switch2 == 3)) {							
							...
							} else if ((finger_count == 1) && (scr_suspended2 == true) && (s2w_switch2 == 1)) {							
								prevx2 = 30;
								nextx2 = 300;
								if ((barrier2[0] == true) ||
							...
							} else if ((finger_count == 1) && (scr_suspended2 == true) && (s2w_switch2 == 4)) {							
								prevx2 = 30;
								nextx2 = 400;
								if ((barrier2[0] == true) ||
								   ((finger_data[loop_i][0] > prevx2) &&
...
if (wake_lock_active(&sweep2wake_wake_lock)) {
				...
				printk(KERN_INFO "[sweep2wake]: resume, release wakelock \n");
			}
	if (s2w_switch2 > 0) {
